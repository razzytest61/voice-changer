...
class RMVPEModule(torch.nn.Module):
    ...
    def forward(self, audio: torch.Tensor):
        mel: torch.Tensor = self.mel_extractor(audio, center=True)
...
if __name__ == '__main__':
    ...
    net_g_onnx = RMVPEModule()
    cpt = torch.load(r'C:\Sources\voice-changer\server\pretrain\rmvpe.pt', map_location='cpu')
    net_g_onnx.e2e.load_state_dict(cpt, strict=False)
    net_g_onnx.eval().to(dev)

    audio = torch.randn(1, 114514, dtype=torch.float32, device=dev).clip(min=-1., max=1.)
    ...
    onnx.save(onnx_model, 'rmvpe_test.onnx')
